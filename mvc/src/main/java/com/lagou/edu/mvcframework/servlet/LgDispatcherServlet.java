...
//        method.invoke() //
     ...
     * @param handler
     */
    private boolean privilegeCheck(Handler handler,HttpServletRequest req) throws IOException {
        ...
        boolean flag=true;
         ...
         {
             Security obj=handler.getController().getClass().getAnnotation(Security.class);
             String[] users=obj.value();
             if(!ArrayUtils.contains(users,userName)||userName==null)
             {
                flag=false;
                return flag;
             }
        ...
        {
            Security obj=handler.getMethod().getAnnotation(Security.class);
            String[] users=obj.value();
            if(!ArrayUtils.contains(users,userName)||userName==null)
            {
                flag=false;
